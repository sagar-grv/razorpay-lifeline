...
async def ladder_followup(payment_id: str, touch_number: int, phone_number: str, recovery_link: str):
    ...
    delay_default = 120 if touch_number == 2 else 900
...
def is_bot_message(text: str) -> bool:
    """Checks if message is generated by Lifeline bot to avoid loop reflection."""
    if not text:
        return False
...
# ----------------- INBOUND EVOLUTION POLLING DAEMON ----------------- #

async def evolution_polling_daemon():
    """Background polling daemon to guarantee catching STOP and Promise-to-Pay replies even if webhooks fail."""
    ...
    seen_poll_msg_ids = set()
    
    # Pre-populate seen_poll_msg_ids on startup to avoid re-triggering historical messages
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if resp.status_code == 200:
                data = resp.json()
                records = data.get("messages", {}).get("records", []) if isinstance(data.get("messages"), dict) else data.get("messages", [])
                if isinstance(records, list):
                    for m in records:
                        mid = m.get("key", {}).get("id")
                        if mid:
                            seen_poll_msg_ids.add(mid)
    except Exception as e:
        log_event(f"[POLLING] Initial sync warning: {safe_log(str(e))}", level="WARN")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                for m in records:
                    key = m.get("key", {})
                    msg_id = key.get("id")
                    if not msg_id or msg_id in seen_poll_msg_ids or msg_id in SEEN_INBOUND_MSG_IDS:
                        continue
                    seen_poll_msg_ids.add(msg_id)
                    SEEN_INBOUND_MSG_IDS.add(msg_id)
                    if len(seen_poll_msg_ids) > 1000:
                        seen_poll_msg_ids.pop()
                    if len(SEEN_INBOUND_MSG_IDS) > 1000:
                        SEEN_INBOUND_MSG_IDS.pop()
                    ...
                    log_event(f"[POLLING] Caught inbound from {safe_log(phone)}: {safe_log(text)}", level="INFO")

                    db = SessionLocal()
                    try:
                        handle_inbound_compliance_intent(phone, text, db, remote_jid=remote_jid)
                    except Exception as err:
                        log_event(f"[POLLING] Error processing intent: {safe_log(str(err))}", level="ERROR")
                    finally:
                        db.close()
        except Exception as e:
            # Wrap entire loop in try/except to prevent crashes
            pass

@app.on_event("startup")
async def startup_event():
    asyncio.create_task(evolution_polling_daemon())
    log_event("[INBOUND MODE] Polling daemon started as webhook fallback", level="SUCCESS")
