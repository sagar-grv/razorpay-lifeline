    ...
    <div className="fintech-card p-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-slate-800">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-white">Failure Code Analysis & Recovery Matrix</h3>
            <p className="text-xs text-slate-400">Autonomous strategy routing across error classifications</p>
          </div>
        </div>
        <div className="flex items-center gap-3 text-xs text-slate-400 font-mono text-[11px]">
          <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block" /> Recovered</span>
          <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> Escalated</span>
          <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-rose-400 inline-block" /> Lost</span>
        </div>
      </div>

      {failureKeys.length === 0 ? (
        <div className="py-12 text-center text-slate-500 text-xs font-mono">
          No transaction events logged yet. Use the Sandbox controls below to simulate failure events.
        </div>
      ) : (
            ...
            return (
              <div key={key} className="p-4 rounded-xl bg-slate-900/60 border border-slate-800/80 hover:border-slate-700 transition-all">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium border font-mono ${cfg.badgeColor}`}>
                    {cfg.action}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 font-mono-numbers">
                  ...
                  <span className="text-slate-300 font-semibold">{data.recovered}/{data.total} ({recPercent}% Saved)</span>
                </div>
              </div>
            );
          })}
